...
<div class="card">
    <div class="card-body">
        <form action="{{route($route.'update', $page->id)}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="col-sm-2">
                    <div class="image-wrapper">
                        <div class="lfm-image-preview"
                            style="--image:url('{{ ($page->image? url($page->image) : asset('images/no-image.png') ) }}')">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input id="image" type="text"
                                ...
                                value="{{ ($page->image ? $page->image : old('image')) }}"
            ...
            <div class="form-group">
                ...
                <input id="name" type="text" class="form-control form-control-sm @error('name') is-invalid @enderror"
                    name="name"
                    value="{{ (old('name') ? old('name') : ($page->name ? $page->name : old('name') ) ) }}"
            ...
            <div class="form-group">
                <label for="">For Static Page </label>

                <input id="for_static_page" type="text" class="form-control form-control-sm @error('for_static_page') is-invalid @enderror"
                    name="for_static_page"
                    value="{{ (old('for_static_page') ? old('for_static_page') : ($page->for_static_page ? $page->for_static_page : old('for_static_page') ) ) }}"
                    placeholder="Enter the for_static_page" autofocus>

                @error('for_static_page')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
                @enderror
            ...
            <div class="form-group">
                ...
                <textarea name="content"
                    class="summernote">{{ (old('content') ? old('content') : ($page->content ? $page->content : old('content') ) ) }}</textarea>
            </div>
